  ...
  return (
    <>
      <section className={style.hero}>
        ...
        <div className={style.content} onMouseMove={({ clientX: x, clientY: y }) => set({ xy: calc(x, y) })} onMouseLeave={()=>set({ xy: [0,0]})}>
          <animated.div style={{ transform: props.xy.interpolate(trans1) }}>
          <h1 className={style.firstLine}>HEY! I'm Bernardo </h1>
          </animated.div>
          <animated.div style={{ transform: props.xy.interpolate(trans2) }}>
          <div className={style.secondLine}> a Web developer looking for job</div>
          </animated.div>
          <animated.div style={{ transform: props.xy.interpolate(trans3) }}>
          <div className={style.heroButton}> <Link href='/portfolio'>Check my Portfolio</Link> </div>
          </animated.div>
        </div>
        </animated.div>
